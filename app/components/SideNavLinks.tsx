...
import { NAVIGATION, CAROUSEL } from '@/app/lib/constants';
...
export default function SideNavLinks() {
    ...
    return (
        <nav aria-label="Mobile menu">
            <FaBars
                className="hidden w-6 max-sm:inline-block"
            ...
            >
                <div className="absolute inset-0 bg-[#1a1f32] p-6">
                    ...
                    <ul className="mt-6 space-y-4 text-sm text-[#ffffff99]">
                        {navItems.map(({ id, label }) => (
                            <li key={id}>
                                <Link
                                    href={`#${id}`}
                                    onClick={() => setIsOpen(false)}
                                    className="flex justify-between border-b border-[#ffffff0d] py-4"
                                >
                                    <span>{label}</span>
